...
- `RealRouteData`: carga `assets/real_routes.json` (275 líneas / 1024 ramales reales de jurisdicción nacional CNRT en el AMBA — 138/566 del dataset KML de datos.transporte.gob.ar más 137/458 extraídas del GTFS de la Ciudad de 2019 para cerrar el hueco de cobertura) en background (`Dispatchers.IO`, vía un `Deferred` arrancado al construir `MockTransportProvider`) y simula un vehículo por ramal moviéndose sobre el trazado real. Reemplaza al viejo `SyntheticFleetGenerator` (recorridos radiales inventados alrededor de un centro fijo): las recomendaciones de línea ahora reflejan geometría real, así funcionan igual de bien en cualquier parte del país, no solo cerca de un centro hardcodeado.
- `ExpectedFrequencyData`: carga `assets/expected_frequency.json` (frecuencia esperada por línea/día/franja horaria, 273 de las 275 líneas, del mismo GTFS de 2019) — complementa a `RealRouteData`, que es solo geometría. Se muestra como referencia honesta ("normalmente cada X min"), nunca para calcular "a horario/demorado" contra una posición simulada.
